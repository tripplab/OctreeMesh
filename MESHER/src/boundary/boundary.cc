 ...
 *@param[in] percentage Is the percentage of the octree used in order to reach the desired 
 *           resolution in the mesh
 */
Scaler::Scaler( double* min_coord , double* max_coord , double percentage ){
	...
	this->SetProportions( percentage );
}
 ...
 *@param[in] percentage Is the percentage of the octree that will be used in the mesher
 *           to reach the desired reoslution
 */
void Scaler::SetProportions( double percentage ){
	double max = -1e20;
	int index = -1;
	...
	for(  int i_dim = 0  ;  i_dim < 3  ;  i_dim++  ){
		if(  i_dim == index  ){
			proportion_[ i_dim ] = percentage;
		}else{
			proportion_[ i_dim ] = domain_[ i_dim ] / domain_[ index ]*percentage;
		}
	}
	for(  int i_dim = 0  ;  i_dim < 3  ;  i_dim++  ){
...
void Boundary::CreatesScalerClass(){
	...
	//Instancin scaler
	scale_ = new Scaler( min_coord , max_coord , 0.99 );
}
