 ...
 *@param is_loaded_ Is a bool value indicating if cell is used as a loaded cell in the 
 *									boundary conditions
 *@param is_fixed_ Is a bool value indicating is cell is used as a fix cell in the 
 *								 boundary conditions
 ...
 *@param printed_fixed_ Is a bool variable indicating if node fixed was printed to file or not
 *@param vertex_distance_ Is the distance between the cell center and the capsid center,
 *                        this information is used to set loaded and fixed elements
 */
struct DataInsideOctreeCell{
	private:
  	...
  	char is_mesh_;
		...
		int r_index_[ 8 ];
		bool printed_[ 8 ];
		bool printed_fixed_[ 8 ];
		double vertex_distance_;
	...
	public:
  	...
  	~DataInsideOctreeCell();
		...
		void _SetPrintedFixed( int node );
		void _SetLoadedElement( double V_distance );
		void _SetNotLoadedElement( );
		void _SetFixedElement( double V_distance );
		void _SetNotFixedElement( );

		//UTILITIES
		bool _IsSetNode( int node );
		bool _NodeWasPrinted(  int node  );
		bool _NodeFixedWasPrinted(  int node  );
		bool _IsLoaded(  );
		bool _IsFixed(  );
