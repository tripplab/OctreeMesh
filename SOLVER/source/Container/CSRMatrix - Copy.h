...
class CSRMatrix
{
	public:
		...
		void CopyRowIndexes(int i, const CSRMatrix& matrix) throw()
		{
			...
			int* __restrict dst = &index[i][1];
			int* __restrict src = &matrix.index[i][1];
			for (int k = count[i]; k; --k, ++dst, ++src)
			{
				*dst = *src;
			}
		}
		...
		void CopyRowValues(int i, const CSRMatrix& matrix) throw()
		{
			...
			T* __restrict dst = &entry[i][1];
			T* __restrict src = &matrix.entry[i][1];
			for (int k = count[i]; k; --k, ++dst, ++src)
			{
				*dst = *src;
			}
		...
		inline T& Entry(int i, int k) const throw()
		{
			Assert(i >= 1);
			Assert(i <= rows);
			Assert(k >= 1);
		...
		void Fill(const T& value) throw()
		{
			...
			for(int i = 1; i <= rows; ++i)
			{
				T* __restrict entry_row = entry[i];
		...
		inline int& Index(int i, int k) const throw()
		{
			Assert(i >= 1);
			Assert(i <= rows);
			Assert(k >= 1);
		...
		void RemoveEntry(int i, int j) throw()
		{
			...
			{
				{
					T* __restrict src = &entry[i][k + 1];
					T* __restrict dst = &entry[i][k];
					for (int l = count[i] - k; l; --l, ++dst, ++src)
					{
						*dst = *src;
					}
				}
				{
					int* __restrict src = &index[i][k + 1];
					int* __restrict dst = &index[i][k];
					for (int l = count[i] - k; l; --l, ++dst, ++src)
					{
						*dst = *src;
					}
