...
class KDTreeB
{
	public:
		...
		KDTreeB(const Vector<T>& values, const Matrix<float>& coordinates) throw(Memory::Exception)
		:	root((Node*)0),
		...
		KDTreeB(const Vector<T>& values, const Matrix<double>& coordinates) throw(Memory::Exception)
		:	root((Node*)0),
			nodes(new Node[coordinates.rows])
		{
			Assert(values.size == coordinates.rows);
		...
		T& Nearest(const K coordinate[D], K& distance_min) throw()
		{
			Assert(root);

			distance_min = Float<K>::infinite;
			Node* nearest = (Node*)0;
	...
	private:
		...
		struct Node
		{
			...
			{
				K keys[D];
				typename FloatFlip<K>::type bits[D];
			};
			T value;


			void Nearest(const K point[D], K& distance_min, Node*& nearest, int depth) throw()
			{
...
//printf("%i\n", value);
		...
		template <>
		void RadixSort<float>(Node** index, int size, int k) throw(Memory::Exception)
		{
			uint32* array = new uint32[size];
			uint32* array_b = new uint32[size];
		...
		template <>
		void RadixSort<double>(Node** index, int size, int k) throw(Memory::Exception)
		{
