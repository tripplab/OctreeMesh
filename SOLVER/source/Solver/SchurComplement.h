...
#ifndef _SchurComplement_h_
#define _SchurComplement_h_
...
class SchurComplementMaster
{
	public:
		...
		SchurComplementMaster(const MPI& mpi, const Vector<Substructure>& substructures, const Boundary& boundary, const int degrees_of_freedom) throw(Memory::Exception, MPI::Exception)
		:	mpi(mpi),
		...
		void Solver(CSRMatrix<T>& KBB, Vector<T>& uB, Vector<T>& fB, T tolerance, int max_steps, int threads) throw(Memory::Exception, MPI::Exception)
		{
			try
			{
				...
				while (step < max_steps)
				{
					...
					for (int i = 1; i <= B; ++i)
					{
						uB.entry[i] += alpha*p.entry[i]; // Xn = uB + alpha*p
	...
	private:
		...
		SchurComplementMaster& operator = (SchurComplementMaster&)
		{
			return *this;
		}
};
...
class SchurComplementSlave
{
	public:
		...
		SchurComplementSlave(const MPI& mpi, const int threads) throw(MPI::Exception)
		:	mpi(mpi),
			Mi(),
			Ii(),
			KIBi(),
		...
		void ReceiveSystemOfEquations() throw(Memory::Exception, MPI::Exception)
		{
			try
			{
				// Receive and factorize system of equations
				{
					...
					FillCholeskyDecomposition(KIIi, L, U, threads);
				}
	...
	private:
		...
		SchurComplementSlave& operator = (SchurComplementSlave&) throw()
		{
			return *this;
		}
};
