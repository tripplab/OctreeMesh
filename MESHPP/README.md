...
- `align_axes:simple` exports the first ten elements found in the input mesh, along with their referenced nodes, as GiD `.post.msh` output
...
For `align_axes:pca`, PCA axes must be non-degenerate. For `align_axes:simple`, element order follows the input file and no geometric alignment is performed. The dominant principal direction aligns with global Z, the second with global X, and the third with global Y.
