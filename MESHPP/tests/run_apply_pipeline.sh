...
rg -n "^mesh\.align_axes\.snap=on$" "$ALIGN_KABSCH_SNAP_STDOUT"
rg -n "^mesh\.align_axes\.snap\.L=1\.000000$" "$ALIGN_KABSCH_SNAP_STDOUT"
rg -n "^mesh\.align_axes\.snap\.unique_planes\.x=2$" "$ALIGN_KABSCH_SNAP_STDOUT"
rg -n "^mesh\.align_axes\.snap\.unique_planes\.y=2$" "$ALIGN_KABSCH_SNAP_STDOUT"
rg -n "^mesh\.align_axes\.snap\.unique_planes\.z=2$" "$ALIGN_KABSCH_SNAP_STDOUT"
rg -n "^mesh\.align_axes\.snap\.cells_collapsed=0$" "$ALIGN_KABSCH_SNAP_STDOUT"
rg -n "^mesh\.align_axes\.snap\.cells_nonunit=0$" "$ALIGN_KABSCH_SNAP_STDOUT"
"$ROUNDTRIP" "$ALIGN_KABSCH_SNAP_OUT" "$TMP_DIR/out_align_axes_kabsch_snap_roundtrip.post.msh" --validate >"$TMP_DIR/out_align_axes_kabsch_snap.validate"
rg -q "nodes: 8" "$TMP_DIR/out_align_axes_kabsch_snap.validate"
rg -q "elements: 1" "$TMP_DIR/out_align_axes_kabsch_snap.validate"
