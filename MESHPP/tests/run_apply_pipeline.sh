...
rg -n "^mesh\.stats\.min\.x=0$" "$STATS_OUT"
rg -n "^mesh\.stats\.max\.z=1$" "$STATS_OUT"
...
rg -n "^mesh\.stats\.min\.x=1$" "$TRANSFORM_STATS_OUT"
rg -n "^mesh\.stats\.min\.z=-1$" "$TRANSFORM_STATS_OUT"
rg -n "^mesh\.stats\.max\.x=3$" "$TRANSFORM_STATS_OUT"
rg -n "^mesh\.stats\.center\.x=2$" "$TRANSFORM_STATS_OUT"
rg -n "^mesh\.stats\.bbox\.diag=3\.4641016151377544$" "$TRANSFORM_STATS_OUT"

FIRST_LINE=$(sed -n "1p" "$STATS_OUT")
SECOND_LINE=$(sed -n "2p" "$STATS_OUT")
THIRD_LINE=$(sed -n "3p" "$STATS_OUT")
if [[ "$FIRST_LINE" != "mesh.stats.nodes=8" || "$SECOND_LINE" != "mesh.stats.elements=1" || "$THIRD_LINE" != "mesh.stats.min.x=0" ]]; then
