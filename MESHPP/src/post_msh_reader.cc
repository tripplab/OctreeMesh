...
ValidationReport PostMshReader::Read(std::istream& input, MeshData* out_mesh) const {
  ...
  while (std::getline(input, line)) {
    ...
    if (StartsWith(t, "MESH")) {
      ...
      if (t.find("ElemType Hexahedra") == std::string::npos) {
        report.issues.push_back({ExitCode::kUnsupported, "E_UNSUPPORTED: only ElemType Hexahedra is supported in v1", line_number});
        return report;
      }
      continue;
    }
    ...
    if (in_coordinates) {
      ...
      if (!(iss >> node.id >> node.xyz[0] >> node.xyz[1] >> node.xyz[2])) {
        report.issues.push_back({ExitCode::kParseError, "E_PARSE: invalid Coordinates row; expected '<id> <x> <y> <z>'", line_number});
        return report;
      }
      if (out_mesh->node_id_to_index.count(node.id) > 0) {
        report.issues.push_back({ExitCode::kParseError, "E_PARSE: duplicate node id in Coordinates", line_number});
        return report;
      }
    ...
    if (in_elements) {
      std::istringstream iss(t);
      HexElement elem;
      if (!(iss >> elem.id >> elem.node_ids[0] >> elem.node_ids[1] >> elem.node_ids[2] >> elem.node_ids[3] >> elem.node_ids[4] >> elem.node_ids[5] >> elem.node_ids[6] >> elem.node_ids[7])) {
        report.issues.push_back({ExitCode::kParseError, "E_PARSE: invalid Elements row; expected '<id> n1..n8>'", line_number});
        return report;
      }
  ...
  if (in_coordinates || in_elements) {
    report.issues.push_back({ExitCode::kParseError, "E_PARSE: unterminated Coordinates or Elements section", line_number});
    return report;
  }
  if (out_mesh->nodes.empty() || out_mesh->elements.empty()) {
    report.issues.push_back({ExitCode::kParseError, "E_PARSE: mesh must contain at least one node and one element", line_number});
    return report;
  }
...
ValidationReport ValidateReferences(const MeshData& mesh) {
  ...
  for (const auto& element : mesh.elements) {
    for (const auto node_id : element.node_ids) {
      if (mesh.node_id_to_index.count(node_id) == 0) {
        report.issues.push_back({ExitCode::kParseError, "E_PARSE: element references undefined node id", 0});
        return report;
      }
    }
  }
